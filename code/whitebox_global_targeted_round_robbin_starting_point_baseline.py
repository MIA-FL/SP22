...
class Organizer():
    ...
    def simple_whitebox_global(self, logger, acc_recorder, attack_recorder, target_recorder, non_target_recorder,\
         train_epoch, global_model, participants, attacker):
        saved_global_model = None
        saved_participants = None

        saved_acc_recorder = None
        saved_attack_recorder = None
        saved_target_recorder = None
        saved_non_target_recorder = None
        ...
        for j in range(train_start,train_epoch+ATTACK_EPOCH):
            if j == (train_epoch):
                ...
                saved_target_recorder = copy.deepcopy(target_recorder)
                saved_non_target_recorder = copy.deepcopy(non_target_recorder)
            ...
            non_target_recorder.loc[len(non_target_recorder)] = (j+1, attack_accuracy, test_acc, pred_acc_member.cpu(), \
                pred_acc_non_member.cpu(), true_member, false_member, true_non_member, false_non_member)
        ...
        non_target_recorder.to_csv(EXPERIMENTAL_DATA_DIRECTORY + "StartPoint" + str(train_epoch) + "_preparation.csv")
        ...
        target_recorder.to_csv(EXPERIMENTAL_DATA_DIRECTORY + "StartPoint" + str(train_epoch) + "_target.csv")
        confidence_recorder.to_csv(EXPERIMENTAL_DATA_DIRECTORY + "StartPoint" + str(train_epoch) + "_confidence.csv")
        ...
        best_target_recorder_index = target_recorder["Target_acc"].idxmax()
        best_target_round = target_recorder["Round"][best_target_recorder_index]
        best_target_acc = target_recorder["Target_acc"][best_target_recorder_index]
        true_target = target_recorder["True_target"][best_target_recorder_index]
        true_member_in_pred_member = target_recorder["True_member"][best_target_recorder_index]
        pred_member = target_recorder["Pred_member"][best_target_recorder_index]

        return saved_acc_recorder, saved_attack_recorder, saved_target_recorder, saved_non_target_recorder, \
            saved_global_model, saved_participants, attacker, (int(train_epoch), int(best_attack_acc_epoch), \
            int(best_attack_acc_epoch-train_epoch), best_attack_acc, target_model_train_acc, target_model_test_acc, \
            ...
            int(best_attack_acc_false_non_member), best_target_round, best_target_acc, true_target, true_member_in_pred_member, pred_member)
    ...
    def federated_training_greykbox_recursive(self, logger, train_epoch, attack_epoch, attack_each_start_recorder):
        ...
        non_target_recorder = None
        
        if train_epoch != 0:
            acc_recorder, attack_recorder, target_recorder, non_target_recorder, global_model, participants, attacker \
            ...
            acc_recorder, attack_recorder, target_recorder, non_target_recorder, global_model, participants, attacker, result = \
                self.simple_whitebox_global(logger, acc_recorder, attack_recorder, target_recorder, non_target_recorder, train_epoch, \
            ...
            attack_each_start_recorder.loc[len(attack_each_start_recorder)] = result
        logger.info("\nStarting_point={}\nbest_attack_acc_in_global_epoch={}\nbest_attack_acc_in_attack_epoch={}\nattack_acc={}\ntarget_model_train_acc={}\ntarget_model_test_acc={}\nprecision={}\nrecall={}\nmember_pred_acc={}\nnon-member_pred_acc={}\ntrue_member={}\nfalse_member={}\ntrue_non_member={}\nfalse_non_member={}\nbest_target_round={}\nbest_target_acc={}\ntrue_target={}\ntrue_member_in_pred_member={}\npred_member={}\n"\
            .format(int(result[0]),int(result[1]),int(result[2]),result[3],result[4],result[5],result[6],\
            result[7],result[8],result[9],int(result[10]),int(result[11]),int(result[12]),int(result[13]),int(result[14]),result[15],result[16],result[17],result[18]))
        return acc_recorder, attack_recorder, target_recorder, non_target_recorder, global_model, participants, attacker
    ...
    def federated_training_simple_whitebox_global_starting_point(self, logger):
        attack_each_start_recorder = pd.DataFrame(columns=["start_epoch","best_in_global_epoch","best_in_attack_epoch", "acc", "target_model_train_acc", \
            ...
            "true_member", "false_member", "true_non_member", "false_non_member", "best_round", \
            "best_target_acc","true_target", "true_member_in_pred_member", "pred_member"])
        ...
        attack_each_start_recorder.to_csv(EXPERIMENTAL_DATA_DIRECTORY + TIME_STAMP + DEFAULT_SET + \
                "_MaxTrainEpoch_" + str(MAX_TRAIN_EPOCH) + "_AttackEpoch_" + str(ATTACK_EPOCH) + "_overview_whitebox_global_target_round_robbin_start_point.csv")
        
        best_attack_index = attack_each_start_recorder["best_target_acc"].idxmax()
        ...
        best_target_acc = attack_each_start_recorder["best_target_acc"][best_attack_index]
        true_target = attack_each_start_recorder["true_target"][best_attack_index]
        true_member_in_pred_member = attack_each_start_recorder["true_member_in_pred_member"][best_attack_index]
        pred_member = attack_each_start_recorder["pred_member"][best_attack_index]

        logger.info("Best result:\nstart_point={}\nattack_acc={}\ntarget_model_train_acc={}\ntarget_model_test_acc={}\nmember_pred_acc={}\nnon-member_pred_acc={}\nbest_attack_acc_epoch={}\nbest_target_round={}\nbest_target_acc={}\ntrue_target={}\ntrue_member_in_pred_member={}\npred_member={}\n"\
            .format(best_start_point, best_attack_acc, target_model_train_acc, target_model_test_acc, \
            member_pred_acc, non_member_pred_acc, best_attack_acc_epoch, best_round, best_target_acc, \
            true_target, true_member_in_pred_member, pred_member))

        return None

if __name__ == '__main__':
    logger = make_logger("project", EXPERIMENTAL_DATA_DIRECTORY, 'log_{}_{}_MAX_TRAIN_EPOCH{}_AttackEpoch_{}_whitebox_global_target_round_robbin_start_point'.format(TIME_STAMP,DEFAULT_SET,str(MAX_TRAIN_EPOCH),str(ATTACK_EPOCH)))
    org = Organizer()
    org.set_random_seed()
