...
for i in range(NUMBER_OF_PARTICIPANTS):
    ...
    shadows.append(FederatedModel(data_reader, aggregator))
    shadows[i].init_global_model()
...
for i in range(NUMBER_OF_PARTICIPANTS):
    participants[i].collect_parameters(global_param)
    shadows[i].collect_parameters(global_param)
for j in range(WHITE_BOX_GLOBAL_TARGETED_ROUND):
    for i in range(NUMBER_OF_PARTICIPANTS):
        aggregator.round_robbin_isolation(participants[i], shadows[i])
        tm, fm, tn, fn = aggregator.round_robbin_evaluation(participants[i], shadows[i])
