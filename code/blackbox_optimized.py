    ...
    def federated_training_nontarget_ada_black_box(self, logger, adaptive=False, record_process=True,
        ...
        for j in range(MAX_EPOCH):
            ...
            if j < TRAIN_EPOCH:
                ...
                # rand_attack = random.randint(0,5)
                # print(rand_attack)
                # if rand_attack == 5 or 3:
                #     if shuffle_factor < 0.5:
                #         shuffle_factor += ADJUST_RATE
                #     else:
                #         pass
                #     attacker.optimized_attack(shuffled_factor=shuffle_factor)
                #     print(shuffle_factor)
                # else:
                #     attacker.optimized_attack(shuffled_factor=0)

                if shuffle_factor < 0.3:
                    # shuffle_factor += ADJUST_RATE
                    attacker.optimized_attack(shuffle_factor=shuffle_factor)
                    logger.info("Current shuffle factor is {}".format(shuffle_factor))
                # elif j % 300 == 0 and shuffle_factor > 0.3:
                #     shuffle_factor = SHUFFLE_FACTOR
                #     attacker.optimized_attack(shuffle_factor=shuffle_factor)
                #     logger.info("Current shuffle factor is {}".format(shuffle_factor))
                #
            ...
            logger.info("Attack accuracy = {}, Precision = {}, Recall={}".format(attack_accuracy, attack_precision,
                                                                                 attack_recall))
            pred_acc_member = attacker.optimized_evaluate_member_accuracy()
            pred_acc_non_member = attacker.optimized_evaluate_non_member_accuracy()
