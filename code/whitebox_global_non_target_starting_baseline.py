...
class Organizer():
    ...
    def federated_training_simple_whitebox_global_starting_point(self, logger):
        ...
        attack_each_start_recorder.to_csv(EXPERIMENTAL_DATA_DIRECTORY + TIME_STAMP + DEFAULT_SET + \
                "_MaxTrainEpoch_" + str(MAX_TRAIN_EPOCH) + "_AttackEpoch_" + str(ATTACK_EPOCH) + "_overview_whitebox_global_nontarget_point.csv")
...
if __name__ == '__main__':
    logger = make_logger("project", EXPERIMENTAL_DATA_DIRECTORY, 'log_{}_{}_MAX_TRAIN_EPOCH{}_AttackEpoch_{}_whitebox_global_nontarget_start_point'.format(TIME_STAMP,DEFAULT_SET,str(MAX_TRAIN_EPOCH),str(ATTACK_EPOCH)))
    org = Organizer()
    org.set_random_seed()
