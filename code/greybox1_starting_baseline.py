...
class Organizer():
    ...
    def simple_greykbox(self, logger, acc_recorder, attack_recorder, train_epoch, global_model, \
    ...
    def federated_training_greykbox_recursive(self, logger, train_epoch, attack_epoch, attack_each_start_recorder):
        ...
        if train_epoch != 0:
            ...
            acc_recorder, attack_recorder, global_model, participants, attacker, result = \
                self.simple_greykbox(logger, acc_recorder, attack_recorder, train_epoch, \
    ...
    def federated_training_greykbox_starting_point(self, logger):
        ...
        attack_each_start_recorder.to_csv(EXPERIMENTAL_DATA_DIRECTORY + TIME_STAMP + DEFAULT_SET + \
                "_MaxTrainEpoch_" + str(MAX_TRAIN_EPOCH) + "_AttackEpoch_" + str(ATTACK_EPOCH) + "_overview_greybox1_start_point.csv")
...
if __name__ == '__main__':
    logger = make_logger("project", EXPERIMENTAL_DATA_DIRECTORY, 'log_{}_{}_MAX_TRAIN_EPOCH{}_AttackEpoch_{}_greybox1_start_point'.format(TIME_STAMP,DEFAULT_SET,str(MAX_TRAIN_EPOCH),str(ATTACK_EPOCH)))
    org = Organizer()
    org.set_random_seed()
    org.federated_training_greykbox_starting_point(logger)